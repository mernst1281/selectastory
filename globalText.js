'use strict';

module.exports = {
	gameEndText : 'Would you like to go back to the beginning and try again?',
	gameEndFallBack : 'Say yes or no',
	newWelcomeFallBack : 'Say play or hear instructions',
	newWelcomeText: 'Welcome to Select-A-Story, where you decide what happens. Do you want to start?',
	optionFallBack : 'Please say A or B',
	resolutionSuccess : '',
	resolutionFail : '',
	refuseText: 'Ok, see you next time! Be sure to visit us at www.selectastory.com',
	returnWelcomeText: 'Welcome Back.  Would you like to continue your game or start over?',
	returnWelcomeFallBack: 'Say continue or start over',
	startOverText: "Let's go back to the beginning",
	unhandled: 'Say yes to continue, help to get more information or no to end the game.',
	helpText : "You are Yanke East's personal assistant and you're trying to find his murderer.  In this game you can follow many paths.  In most cases will be given a choice of two, sometimes three options.  To make a selection either say <emphasis>1</emphasis> or <emphasis>2</emphasis>.  In a few cases you will need to select a person.  Say the full name of the person you are selecting.",
	helpTextFallBack : "Please say <emphasis>again<emphasis> to hear this again or <emphasis>go</emphasis> to continue the game"
};